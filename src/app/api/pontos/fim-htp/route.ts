...
import { registrarFimHtp } from '@/lib/firebaseDb'
import { SYSTEM_CONFIG } from '@/lib/config'

export async function POST(req: NextRequest) {
  try {
    console.log('🕐 API fim-htp iniciada')
    ...
    try {
      ...
      const resultado = await registrarFimHtp(
        usuario.email,
        horaAtual,
        location
      )

      console.log('✅ Fim de HTP registrado com sucesso')
      
      return NextResponse.json({
        success: true,
        message: 'Fim de HTP registrado com sucesso',
        horaFimHtp: horaAtual,
        location: location ? {
          validated: true,
    ...
    console.error('💥 Erro geral na API fim-htp:', error)
