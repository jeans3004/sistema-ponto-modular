...
import { registrarInicioAlmoco } from '@/lib/firebaseDb'
...
export async function POST(req: NextRequest) {
  try {
    ...
    const horaInicioAlmoco = agora.toTimeString().slice(0, 5) // HH:MM

    // Registrar início do almoço no Firebase
    await registrarInicioAlmoco(usuario.email, horaInicioAlmoco, locationData)

    return NextResponse.json({ 
      success: true, 
      horaInicioAlmoco,
      message: 'Início do almoço registrado com sucesso',
    ...
    console.error('Erro na API de início do almoço:', error)
    
    const errorMessage = error instanceof Error ? error.message : 'Erro ao registrar início do almoço'
